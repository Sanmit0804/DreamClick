...
const PageTransition: React.FC<{ children: React.ReactNode; pathKey: string }> = ({ children, pathKey }) => (
    <motion.div
        key={pathKey}
...
const Admin = () => {
    ...
    const generateBreadcrumbs = () => {
        ...
        const crumbs = [{ label: 'Dashboard', href: '/admin', isCurrent: pathname === '/admin' }];
        let cur = '/admin';
        segs.forEach((s, i) => {
            cur += `/${s}`;
    ...
    // Render content based on exact path matching
    const renderContent = () => {
        // Users sub-paths
        if (pathname === '/admin/users') return <PageTransition pathKey={pathname}><ManageUser /></PageTransition>;
        if (pathname.startsWith('/admin/users/')) return <PageTransition pathKey={pathname}><EditUser /></PageTransition>;
                ...
                <Menubar className="flex-col h-full items-stretch border-none">
                                        ...
                                        {item.subItems.map((sub, idx) => (
                                            <React.Fragment key={sub.id}>
                                                <MenubarItem
                                                    ...
                                                    className={`flex items-center gap-2 cursor-pointer ${pathname === sub.path ? 'bg-accent' : ''}`}
                                                >
                                                    <motion.div
                                                        whileHover={{ x: 3 }}
                                                        whileTap={{ scale: 0.96 }}
                    ...
                    <div className="p-4 border-t mt-auto">
                        ...
                        >
                            ...
                            {!isMobile && <span>Back to Website</span>}
                        </Button>
                    </div>
                </Menubar>
            </motion.aside>
