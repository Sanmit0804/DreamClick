                        ...
                        {isLoginScreen ? (
                            ...
                            <form onSubmit={handleSignupSubmit(onSignupSubmit)} className="space-y-4">
                                <Tabs value={currentTab} onValueChange={setCurrentTab} className="w-full">
                                    ...
                                    <TabsContent value="basic" className="space-y-4 mt-4">
                                        <div className="grid gap-2">
                                            ...
                                            <Input id="signup-name" type="text" placeholder="Enter your full name" {...registerSignup("name")} disabled={isLoading} />
                                            {signupErrors.name && <span className="text-xs sm:text-sm text-red-500">{signupErrors.name.message}</span>}
                                        </div>
                                        <div className="grid gap-2">
                                            <Label htmlFor="signup-email">Email *</Label>
                                            <Input id="signup-email" type="email" placeholder="m@example.com" {...registerSignup("email")} disabled={isLoading} />
                                            {signupErrors.email && <span className="text-xs sm:text-sm text-red-500">{signupErrors.email.message}</span>}
                                        </div>
                                        <div className="grid gap-2">
                                            <Label htmlFor="signup-password">Password *</Label>
                                            <PasswordInput id="signup-password" placeholder="At least 6 characters" {...registerSignup("password")} disabled={isLoading} />
                                            {signupErrors.password && <span className="text-xs sm:text-sm text-red-500">{signupErrors.password.message}</span>}
                                        </div>
                                        <div className="grid gap-2">
                                            <Label htmlFor="signup-confirm-password">Confirm Password *</Label>
                                            <PasswordInput id="signup-confirm-password" placeholder="Confirm your password" {...registerSignup("confirmPassword")} disabled={isLoading} />
                                            {signupErrors.confirmPassword && <span className="text-xs sm:text-sm text-red-500">{signupErrors.confirmPassword.message}</span>}
                                        </div>
                                        <div className="grid gap-2">
                                            ...
                                            >
                                                <div className="flex items-center space-x-2 border rounded-lg p-3 hover:bg-accent">
                                                    ...
                                                    <Label htmlFor="end_user" className="flex-1 cursor-pointer">
                                                        ...
                                                        <div className="text-xs text-muted-foreground">Browse and purchase templates</div>
                                                    </Label>
                                                </div>
                                                <div className="flex items-center space-x-2 border rounded-lg p-3 hover:bg-accent">
                                                    ...
                                                    <Label htmlFor="content_creator" className="flex-1 cursor-pointer">
                                                        ...
                                                        <div className="text-xs text-muted-foreground">Create and sell video templates</div>
                                                    </Label>
                                                </div>
                                            </RadioGroup>
                                            {signupErrors.role && <span className="text-xs sm:text-sm text-red-500">{signupErrors.role.message}</span>}
                                        </div>
                                    </TabsContent>
                                    ...
                                    <TabsContent value="contact" className="space-y-4 mt-4">
                                        <div className="grid gap-2">
                                            ...
                                            <Input id="signup-phone" type="tel" placeholder="9876543210" {...registerSignup("phone")} disabled={isLoading} />
                                            {signupErrors.phone && <span className="text-xs sm:text-sm text-red-500">{signupErrors.phone.message}</span>}
                                        </div>
                                        <div className="pt-2">
                                            ...
                                            <div className="space-y-4">
                                                <div className="grid gap-2">
                                                    ...
                                                    <Input id="signup-street" type="text" placeholder="Enter street address" {...registerSignup("billingAddress.street")} disabled={isLoading} />
                                                </div>
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                    <div className="grid gap-2">
                                                        <Label htmlFor="signup-city">City</Label>
                                                        <Input id="signup-city" type="text" placeholder="City" {...registerSignup("billingAddress.city")} disabled={isLoading} />
                                                    </div>
                                                    <div className="grid gap-2">
                                                        <Label htmlFor="signup-state">State</Label>
                                                        <Input id="signup-state" type="text" placeholder="State" {...registerSignup("billingAddress.state")} disabled={isLoading} />
                                                    </div>
                                                </div>
                                                <div className="grid gap-2">
                                                    <Label htmlFor="signup-zip">PIN Code</Label>
                                                    <Input id="signup-zip" type="text" placeholder="400001" {...registerSignup("billingAddress.zipCode")} disabled={isLoading} />
                                                    {signupErrors.billingAddress?.zipCode && <span className="text-xs sm:text-sm text-red-500">{signupErrors.billingAddress.zipCode.message}</span>}
                                                </div>
                                            </div>
                                        </div>
                                    </TabsContent>
                                            ...
                                            <>
                                                <div className="grid gap-2">
                                                    ...
                                                    <textarea
                                                        ...
                                                        disabled={isLoading}
                                                    />
                                                    {signupErrors.creatorProfile?.bio && <span className="text-xs sm:text-sm text-red-500">{signupErrors.creatorProfile.bio.message}</span>}
                                                </div>
                                                <div className="grid gap-2">
                                                    <Label htmlFor="signup-website">Website</Label>
                                                    <Input id="signup-website" type="url" placeholder="https://yourwebsite.com" {...registerSignup("creatorProfile.website")} disabled={isLoading} />
                                                    {signupErrors.creatorProfile?.website && <span className="text-xs sm:text-sm text-red-500">{signupErrors.creatorProfile.website.message}</span>}
                                                </div>
                                                <div className="pt-2">
                                                    ...
                                                    <div className="space-y-3">
                                                        <div className="grid gap-2">
                                                            ...
                                                            <Input id="signup-youtube" type="text" placeholder="@yourchannel or channel URL" {...registerSignup("creatorProfile.socialLinks.youtube")} disabled={isLoading} />
                                                        </div>
                                                        <div className="grid gap-2">
                                                            <Label htmlFor="signup-instagram">Instagram</Label>
                                                            <Input id="signup-instagram" type="text" placeholder="@yourusername" {...registerSignup("creatorProfile.socialLinks.instagram")} disabled={isLoading} />
                                                        </div>
                                                        <div className="grid gap-2">
                                                            <Label htmlFor="signup-twitter">Twitter/X</Label>
                                                            <Input id="signup-twitter" type="text" placeholder="@yourusername" {...registerSignup("creatorProfile.socialLinks.twitter")} disabled={isLoading} />
                                                        </div>
                                                    </div>
                                                </div>
                                            </>
                                ...
                                <div className="flex flex-col sm:flex-row gap-2 pt-4">
                                    ...
                                    {currentTab === "basic" && (
                                        <Button type="button" variant="outline" onClick={() => setCurrentTab("contact")} disabled={isLoading} className="w-full">
                                            Next: Contact Info
                                        </Button>
                                    )}

                                    {currentTab === "contact" && selectedRole === 'content_creator' && (
                                        <Button type="button" variant="outline" onClick={() => setCurrentTab("additional")} disabled={isLoading} className="flex-1">
                                            Next: Profile
                                        </Button>
                                    )}
